...
COLA_MODEL_ROOT = os.path.expanduser("~/.cache/gruen/")
...
# function to load or download model if it's not already downloaded
def load_or_download_spacy_model(model):
    try:
        ...
        nlp = spacy.load(model)
        print(f"{model} model has been downloaded and loaded.")
    return nlp


def prepare_cola_model(root):
    import zipfile

    import gdown

    os.makedirs(COLA_MODEL_ROOT, exist_ok=True)
    gdown.download(id="1Hw5na_Iy4-kGEoX60bD8vXYeJDQrzyj6", output="/tmp/", quiet=False)

    with zipfile.ZipFile("/tmp/cola_model.zip", "r") as zip_ref:
        zip_ref.extractall(COLA_MODEL_ROOT)
...
def get_lm_score(sentences):
    def score_sentence(sentence, tokenizer, model):
        ...
        input_ids = (
            torch.tensor(tokenizer.encode(tokenize_input)).unsqueeze(0).to(device)
        )
        with torch.no_grad():
...
def get_cola_score(sentences):
    def load_pretrained_cola_model(model_name, saved_pretrained_CoLA_model_dir):
        ...
        config = config_class.from_pretrained(
            saved_pretrained_CoLA_model_dir, num_labels=2, finetuning_task="CoLA"
        )
    ...
    def evaluate_cola(model, candidates, tokenizer, model_name):
        def load_and_cache_examples(candidates, tokenizer):
            max_length = 128
            examples = [
                InputExample(guid=str(i), text_a=x) for i, x in enumerate(candidates)
            ]
            ...
            all_token_type_ids = torch.tensor(
                [[0.0] * max_length for f in features], dtype=torch.long
            )
    ...
    model_name = "bert-base-cased"
    saved_pretrained_CoLA_model_dir = COLA_MODEL_ROOT + 'cola_model/' + model_name + "/"
    if not os.path.exists(saved_pretrained_CoLA_model_dir):
        prepare_cola_model(saved_pretrained_CoLA_model_dir)
            ...
            LCS_word_length = len(
                a[
                    longest_common_substring[0] : (
                        longest_common_substring[0] + LCS_string_length
                    )
                ]
                .strip()
                .split()
    ...
    for i in range(len(all_summary)):
        ...
        for j in range(len(summary) - 1):  # for pairwise redundancy
            for k in range(j + 1, len(summary)):
                flag += if_two_sentence_redundant(
                    summary[j].strip(), summary[k].strip()
                )
...
def get_focus_score(all_summary):
    def compute_sentence_similarity():
        ...
        nlp = load_or_download_spacy_model("en_core_web_lg")
...
@validate_arguments
def gruen_score(text: str) -> float:
    return get_gruen([text])

@validate_arguments
def gruen_scores(texts: Sequence[str]) -> List[float]:
    return get_gruen(texts)
